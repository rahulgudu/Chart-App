...
const AuthScreen = () => {
    const { loadingStrategy } = useSocialAuth();
    ...
    return (
        <View className='flex-1 bg-background '>
            ...
            <View className='absolute inset-0'>
                <LinearGradient colors={["#0F0E17", '#1A1A2E', "#2D1B69", "#1A1A2E", "#0F0E17"]} locations={[0, 0.25, 0.5, 0.75, 1]} style={{ width: "100%", height: "100%" }} start={{ x: 0.5, y: 0 }} />
            </View>
            <SafeAreaView className='flex-1 justify-between'>
                ...
                <View>
                    <View className='items-center pt-10 pb-2'>
                        ...
                        <Text className='text-3xl font-extrabold text-foreground tracking-tight mt-4 font-mono'>Chatter-Patter</Text>

                        <Text className='text-foreground-muted text-[15px] mt-1.5 tracking-wide'>Start your chit-chat</Text>
                    </View>
                ...
                <View className='px-8 pb-4'>
                    ...
                    <View className='flex-row justify-center items-center gap-4 mb-5'>
                        {/* Google Button */}
                        <Pressable className='size-14 rounded-2xl bg-white items-center justify-center active:scale-95 shadow-lg shadow-white/10' style={({ pressed }) => ({ opacity: pressed ? 0.85 : 1 })} disabled={isLoading}>
